...
将所有会话事件以 JSON 行输出到 stdout。便于将 pi 集成到其他工具或自定义 UI。
...
`queue_update` 在待处理的 steering 与 follow-up 队列变化时发出完整队列。`compaction_start` 与 `compaction_end` 覆盖手动与自动压缩。
...
`message_update` 记录仅为增量。它们省略累积的 `message` 字段以及
`assistantMessageEvent.partial`，以保持流大小线性。需要时用 `contentIndex` 和 `delta`
组装实时文本、思考或工具调用参数。`message_end` 包含
最终权威消息。
