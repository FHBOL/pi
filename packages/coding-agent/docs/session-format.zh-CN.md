...
会话以 JSONL（JSON Lines）文件存储。每行是一个带 `type` 字段的 JSON 对象。会话条目通过 `id`/`parentId` 字段形成树结构，从而能在不创建新文件的情况下进行就地分支。
...
其中 `<path>` 是将工作目录中的 `/` 替换为 `-` 后的路径。

## 删除会话

可通过删除 `~/.pi/agent/sessions/` 下的 `.jsonl` 文件来移除会话。

Pi 还支持在 `/resume` 中交互式删除会话（选择会话并按 `Ctrl+D`，然后确认）。可用时，pi 使用 `trash` CLI 以避免永久删除。
...
若要在项目中查看 TypeScript 定义，请检查 `node_modules/@earendil-works/pi-coding-agent/dist/` 与 `node_modules/@earendil-works/pi-ai/dist/`。

## 消息类型

会话条目包含 `AgentMessage` 对象。理解这些类型对于解析会话与编写扩展至关重要。
...
导出的 pi-ai `StopReason` 类型也包含 `"pending"`，但该值保留给流式事件中的部分消息。终端 `done`/`error` 消息会在 pi 持久化助手消息之前将其替换为完成原因，因此 `"pending"` 不应出现在会话 JSONL 中。
...
文件的第一行。仅元数据，不属于树（无 `id`/`parentId`）。
...
较新的 harness 生成的压缩会将压缩后保留的上下文直接嵌入条目，而不是使用 `firstKeptEntryId`：
...
可选字段：
- `usage`：生成摘要时的 LLM 用量；计入会话 token 与成本总计
- `retainedTail`：压缩后保留的物化 `AgentMessage[]`。仅对旧会话为可选以保持向后兼容。较新的 harness 生成的压缩会包含它，以便可以从该检查点重建上下文，而无需遍历压缩条目之前的旧条目。
- `details`：实现特定数据（例如默认的 `{ readFiles: string[], modifiedFiles: string[] }`，或扩展的自定义数据）
- `fromHook`：由扩展生成时为 `true`；由 pi 生成时为 `false`/`undefined`（旧字段名）
- `firstKeptEntryId`：与旧条目格式兼容。

### BranchSummaryEntry

通过 `/tree` 切换分支时创建，带有由 LLM 生成的、从离开分支到公共祖先的摘要。捕获被放弃路径的上下文。
...
可选字段：
- `usage`：生成摘要时的 LLM 用量；计入会话 token 与成本总计
- `details`：文件跟踪数据（默认 `{ readFiles: string[], modifiedFiles: string[] }`，或扩展自定义数据）
- `fromHook`：由扩展生成时为 `true`；由 pi 生成时为 `false`/`undefined`（旧字段名）
...
使用 `customType` 在重载时识别扩展的条目。交互模式可通过 `pi.registerEntryRenderer(customType, renderer)` 渲染自定义条目，但它们仍不参与 LLM 上下文。
...
设置后，会话名称会在会话选择器（`/resume`）中替代首条消息显示。

## 树结构

条目形成树：
- 第一条目的 `parentId` 为 `null`
...
`buildContextEntries()` 从当前叶子走到根，在遵循压缩规则的同时生成活动条目列表：

1. 收集路径上的所有条目
2. 若路径上有 `CompactionEntry`：
   - 首先包含压缩条目
   - 若存在 `retainedTail`，它作为自包含检查点，并包含压缩之后的条目
   - 否则包含从 `firstKeptEntryId` 到压缩的条目
   - 然后包含压缩之后的条目
3. 在选定范围内保留非消息条目，以便交互模式可以渲染它们

`buildSessionContext()` 基于该条目列表构建供 LLM 使用的消息列表：

1. 从完整路径提取当前模型与思考级别设置
2. 将选定条目转换为消息：
...
这使较新的压缩像自包含检查点一样工作。`retainedTail` 仅在可选时是为了让只存储 `firstKeptEntryId` 的旧会话继续正确加载。
...
- `SessionManager.forkFrom(sourcePath, targetCwd, sessionDir?)` - 从另一项目派生会话
...
- `buildSessionContext()` - 获取供 LLM 使用的消息、thinkingLevel 与模型
